...
for regionNum = 1:size(regionsStats, 1)
    ...
    % Check if element found is withing image's range
    if round(regionsStats(regionNum).BoundingBox(1, 2) +  ...
             regionsStats(regionNum).BoundingBox(1, 4)) <= size(RGB, 1) && ...
       round(regionsStats(regionNum).BoundingBox(1, 1) +  ...
             regionsStats(regionNum).BoundingBox(1, 3)) <= size(RGB, 2)
